...
async function loadSession() {
    ...
    // FORMAT 2: NOVA~ (MEGA.nz)
    else if (config.SESSION_ID.startsWith("NOVA~")) {
        console.log("📥 Detected NOVA~ format session (MEGA)");
        const sessdata = config.SESSION_ID.replace("NOVA~", '');
        
        try {
            const filer = File.fromURL(`https://mega.nz/file/${sessdata}`);
            
            // Use Promise for async download
...
//=============================================
// Status reactions cache (for rate limiting) - REDUCED COOLDOWN FOR SPEED
const statusReactCache = new Map();
const statusReactCooldown = 1000; // Reduced to 1 second for faster reactions
...
async function connectToWA() {
  try {
    // Load session first (supports both formats)
    await loadSession();
...
> *© ᴘᴏᴡᴇʀᴇᴅ ʙʏ 𝙽𝙾𝚅𝙰 ᴛᴇᴄʜ*`,
    ...
    conn.ev.on('messages.upsert', async(mek) => {
      ...
      // ✅ STATUS HANDLER - FIXED: SINGLE EMOJI "🤝" WITH FAST REACTION
      if (mek.key && mek.key.remoteJid === 'status@broadcast') {
        ...
        // 2. Auto-react to status - FAST REACTION WITH SINGLE EMOJI "🤝"
        if (config.AUTO_STATUS_REACT === "true") {
          
          const now = Date.now()
          const lastReact = statusReactCache.get(mek.key.participant) || 0
          
          // REDUCED COOLDOWN FOR FASTER REACTIONS (1 second instead of 3)
          if (now - lastReact > statusReactCooldown) {
            try {
              const botJid = conn.user.id.split(':')[0] + '@s.whatsapp.net'
              
              // SINGLE EMOJI ONLY - 🤝 (same as the first code)
              const statusEmoji = "🤝"
