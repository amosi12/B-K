const {
  ...
  Browsers,
  isJidNewsletter  // ✅ Added for newsletter detection
...
// ✅ Newsletter Configuration (ONGEZA HAPA)
const NEWSLETTER_JID = '120363420670416756@newsletter'; // Badilisha na newsletter ID yako
const NEWSLETTER_NAME = 'NOVA-XMD';
const AUTO_FOLLOW_NEWSLETTER = true;  // Weka true kuactivate auto-follow
const AUTO_REACT_NEWSLETTER = true;   // Weka true kuactivate auto-react
const REACT_EMOJIS = ['🤖', '🔥', '💯', '❤️', '👍', '💫', '✨', '👏', '😎', '🚀', '🌟', '🎯', '⚡', '💥', '🎉']; // Emoji za reaction
...
// ✅ Function to follow newsletter (ONGEZA HAPA)
async function followNewsletter(conn) {
    ...
    try {
        ...
        if (typeof conn.newsletterFollow === 'function') {
            ...
            console.log(chalk.green(`✅ Successfully followed newsletter: ${NEWSLETTER_JID}`));
            
            // Get newsletter info to verify
            try {
                const info = await conn.newsletterInfo(NEWSLETTER_JID);
                if (info && info.name) {
                    console.log(chalk.cyan(`📰 Newsletter Name: ${info.name}`));
                }
            } catch (infoErr) {
                // Ignore info fetch errors
            }
        } else {
            console.log(chalk.yellow(`⚠️ newsletterFollow method not available in this Baileys version`));
            
            // Alternative method using newsletter action
            try {
                await conn.newsletterAction('follow', NEWSLETTER_JID);
                console.log(chalk.green(`✅ Successfully followed newsletter via action: ${NEWSLETTER_JID}`));
            } catch (altErr) {
                console.log(chalk.yellow(`⚠️ Alternative follow method also failed`));
            }
        }
    } catch (err) {
...
// ✅ Function to handle newsletter messages with auto-reaction (ONGEZA HAPA)
async function handleNewsletterMessage(conn, message, key) {
    ...
    try {
        ...
        console.log(chalk.green(`✅ Auto-reacted with ${randomEmoji} to newsletter post`));
        
    } catch (err) {
        console.error(chalk.red(`❌ Newsletter reaction failed: ${err.message}`));
    }
}
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          // ✅ Auto-follow newsletter when connection opens (ONGEZA HAPA)
          await followNewsletter(conn);
...
║
║ 📰 Auto-Follow  : ${AUTO_FOLLOW_NEWSLETTER ? '✅' : '❌'}
║
║ 🤖 Auto-React   : ${AUTO_REACT_NEWSLETTER ? '✅' : '❌'}
║
║ ⭐ Support     : Fork ⭐ & Star 🔥
║
...
> *© ᴘᴏᴡᴇʀᴇᴅ ʙʏ 𝙽𝙾𝚅𝙰 ᴛᴇᴄʜ*`,
    ...
    conn.ev.on('messages.upsert', async(mek) => {
      ...
      // ✅ NEW: Check for newsletter messages and auto-react (ONGEZA HAPA)
      if (mek.key && mek.key.remoteJid === NEWSLETTER_JID) {
        console.log(chalk.cyan(`📬 Newsletter message received from ${NEWSLETTER_JID}`));
        await handleNewsletterMessage(conn, mek.message, mek.key);
        // Optional: return if you don't want newsletter messages to be processed further
        // return;
      }
      
      // ✅ FIXED STATUS HANDLER (Using POPKID-MD style)
      if (from === 'status@broadcast') {
        try {
          ...
          if (statusParticipant) {
            ...
            // Handle LID (WhatsApp internal ID format)
            ...
            // Auto-react to status (with cooldown)
            if (shouldReact) {
              const now = Date.now()
              const lastReact = statusReactCache.get(realJid) || 0
              ...
              if (now - lastReact > statusReactCooldown) {
                ...
                if (reactableTypes.includes(type)) {
                  // Simple but beautiful emoji list
                  const emojis = [
                    '💓', '🍉', '🇹🇿', '😊', '🔁', '💊', '💫', '🌚', 
                    '🌟', '🎋', '🫀', '😎', '🤖', '🚩', '🥰', '🙄',
                    '✨', '🔥', '💯', '✅', '❤️', '💖', '💯', '⭐'
                  ...
                  statusReactCache.set(realJid, now);
                  console.log(`✅ Status reaction sent: ${emoji} to ${realJid.split('@')[0]}`);
                }
              }
            }
            ...
            if (config.AUTO_STATUS_REPLY === "true") {
              ...
              // Don't quote the status message when replying
              await conn.sendMessage(realJid, { text: replyMsg });
            }
          }
        } catch (e) { 
        ...
        return; // Important: return after handling status
      }
