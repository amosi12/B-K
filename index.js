    ...
    if (config.SESSION_ID && config.SESSION_ID.startsWith("NOVA;;;")) {
        ...
        try {
            ...
            if (compressedBuffer[0] === 0x1f && compressedBuffer[1] === 0x8b) {
                ...
                console.log("Session restored from NOVA~ format ✅");
                return true;
            } else {
                await fs.promises.writeFile(credsPath, compressedBuffer.toString('utf-8'));
                console.log("Session restored from raw Base64 ✅");
                return true;
            }
        } catch (error) { 
            console.error("Failed to decompress session ID", error);
            return false; 
        }
    } else {
